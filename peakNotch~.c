/**********************************************************
 * PeakNotch filter external, based on Mike Moser-Booth's *
...
// The class pointer
static t_class *peakNotch_class;

// The object structure
typedef struct _peakNotch {
	// The Pd object
	t_object obj;
...
// Function prototypes
void *peakNotch_new(void);
void peakNotch_dsp(t_peakNotch *x, t_signal **sp);
t_int *peakNotch_perform(t_int *w);

// The Pd class definition function
void peakNotch_tilde_setup(void)
{
	// Initialize the class
	peakNotch_class = class_new(gensym("peakNotch~"), (t_newmethod)peakNotch_new, 0, sizeof(t_peakNotch), 0, 0);

	// Specify signal input, with automatic float to signal conversion
	CLASS_MAINSIGNALIN(peakNotch_class, t_peakNotch, x_f);

	// Bind the DSP method, which is called when the DACs are turned on
	class_addmethod(peakNotch_class, (t_method)peakNotch_dsp, gensym("dsp"), A_CANT, 0);

	// Print authorship to Pd window
	post("peakNotch~: Filter external based on [filtercoeff.mmb~] and [biquad.mmb~] abstractions\nby Mike Moser-Booth, written by Alexandros Drymonitis");
}
...
void *peakNotch_new(void)
{
	...
	t_peakNotch *x = (t_peakNotch *) pd_new(peakNotch_class);
	...
	// Initialize internal state variables to zero
	x->x_last_in = 0;
	x->x_prev_in = 0;
	x->x_last_out = 0;
	x->x_prev_out = 0;
...
t_int *peakNotch_perform(t_int *w)
{
	...
	float sr = x->x_sr;
	// Local variables
	float freq_local, bandwidth_local, gain_local;
	...
	while(n--){
		...
		freq_cos = (-2) * cos(freq_local);
		alpha = sin(freq_local) / (2 * bandwidth_local);

		// Coefficient values
		coeff_a0 = (alpha / gain_local) + 1;
		coeff_a1 = freq_cos;
		coeff_a2 = 1 - (alpha / gain_local);
		coeff_b0 = (alpha * gain_local) + 1;
		coeff_b1 = freq_cos;
		coeff_b2 = 1 - (alpha * gain_local);
...
void peakNotch_dsp(t_peakNotch *x, t_signal **sp)
{
	...
	/* Attach the object to the DSP chain, passing the DSP routine peakNotch_perform(),
	inlet and outlet pointers, and the signal vector size */
	dsp_add(peakNotch_perform, 7, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec, sp[0]->s_n);
}
