/*****************************************************************
 * High shelf filter external, combination of Mike Moser-Booth's *
...
// Function prototypes
void *highShelf_new(void);
void highShelf_dsp(t_highShelf *x, t_signal **sp);
t_int *highShelf_perform(t_int *w);

// The Pd class definition function
void highShelf_tilde_setup(void)
{
	// Initialize the class
	highShelf_class = class_new(gensym("highShelf~"), (t_newmethod)highShelf_new, 0, sizeof(t_highShelf), 0, 0);

	// Specify signal input, with automatic float to signal conversion
	CLASS_MAINSIGNALIN(highShelf_class, t_highShelf, x_f);

	// Bind the DSP method, which is called when the DACs are turned on
	class_addmethod(highShelf_class, (t_method)highShelf_dsp, gensym("dsp"), A_CANT, 0);

	// Print authorship to Pd window
	post("highShelf~: Filter external based on [filtercoeff.mmb~] and [biquad.mmb~] abstractions\nby Mike Moser-Booth, written by Alexandros Drymonitis");
}
...
void highShelf_dsp(t_highShelf *x, t_signal **sp)
{
	...
	/* Attach the object to the DSP chain, passing the DSP routine highShelf_perform(),
	inlet and outlet pointers, and the signal vector size */
	dsp_add(highShelf_perform, 7, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec, sp[4]->s_vec, sp[0]->s_n);
}
