/****************************************************************
 * High pass filter external, combination of Mike Moser-Booth's *
...
// Function prototypes
void *highPass_new(void);
void highPass_dsp(t_highPass *x, t_signal **sp);
t_int *highPass_perform(t_int *w);

// The Pd class definition function
void highPass_tilde_setup(void)
{
	// Initialize the class
	highPass_class = class_new(gensym("highPass~"), (t_newmethod)highPass_new, 0, sizeof(t_highPass), 0, 0);

	// Specify signal input, with automatic float to signal conversion
	CLASS_MAINSIGNALIN(highPass_class, t_highPass, x_f);

	// Bind the DSP method, which is called when the DACs are turned on
	class_addmethod(highPass_class, (t_method)highPass_dsp, gensym("dsp"), A_CANT, 0);

	// Print authorship to Pd window
	post("highPass~: Filter external based on [filtercoeff.mmb~] and [biquad.mmb~] abstractions\nby Mike Moser-Booth, written by Alexandros Drymonitis");
}
...
t_int *highPass_perform(t_int *w)
{
	...
	while(n--){
		...
		coeff_a1 = freq_cos * (-2);
		coeff_a2 = 1 - alpha;
		coeff_b0 = (freq_cos + 1) / 2;
		coeff_b1 = (freq_cos + 1) * (-1);
		coeff_b2 = (freq_cos + 1) / 2;
...
void highPass_dsp(t_highPass *x, t_signal **sp)
{
	...
	/* Attach the object to the DSP chain, passing the DSP routine highPass_perform(),
	inlet and outlet pointers, and the signal vector size */
	dsp_add(highPass_perform, 6, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec, sp[0]->s_n);
}
